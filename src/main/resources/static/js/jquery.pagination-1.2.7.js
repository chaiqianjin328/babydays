...
!function ($) {
    ...
    var Page = function (element, options) {
        ...
        this.$jump = $('<div id="queryJump" class="m-pagination-jump"></div>');
        ...
        var init = function (obj) {
            ...
            //init jump module
            var jumpHtml = '<div class="m-pagination-group"><input id="queryIput" type="text"><button id="queryBtn" data-page-btn="jump" type="button">' + that.options.jumpBtnText + '</button></div>';
    ...
    Page.prototype = {
        _remoteOrRedner: function (pageIndex) {
            if (this.options.remote.url != null && (this.options.loadFirstPage || pageIndex > 0))
            	//这里添加了参数
                this.remote(pageIndex,{query:$("#query").val(),gardenId:$("#queryGardenId").val(),classId:$("#queryClassId").val(),type:$("#queryStudentType").val(),stuId:$("#queryStuId").val(),createtime:$("#queryDate").val(),leaveday:$("#leaveday").val(),start:$("#queryStart").val(),end:$("#queryEnd").val()});
            else
